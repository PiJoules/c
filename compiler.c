...
void union_type_dump(const Type*);

const TypeVtable UnionTypeVtable = {
    .kind = TK_UnionType,
...
const NodeVtable UnionDeclarationVtable = {
    .kind = NK_UnionDeclaration,
...
void pretty_function_destroy(Expr* expr) {}

const ExprVtable PrettyFunctionVtable = {
...
const ExprVtable StmtExprVtable = {
    .kind = EK_StmtExpr,
...
Expr* parse_cast_or_paren_or_stmt_expr(Parser* parser) {
  size_t line = parser_peek_token(parser)->line;
  size_t col = parser_peek_token(parser)->col;
...
Node* parse_union_declaration(Parser* parser) {
  char* name = NULL;
  vector* members = NULL;
  parse_union_name_and_members(parser, &name, &members);
